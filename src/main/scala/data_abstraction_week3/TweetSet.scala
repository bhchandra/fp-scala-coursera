...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    left.filterAcc(p, acc2) union right.filterAcc(p, acc2)
  }
  ...
  def mostRetweeted: Tweet = {
    if (!left.isEmpty && !right.isEmpty) {
      ...
      val mostRight: Tweet = right.mostRetweeted
      if (elem.retweets >= mostRight.retweets) elem else mostRight
    }
    else if (!left.isEmpty && right.isEmpty) {
      val mostLeft: Tweet = left.mostRetweeted
      if (elem.retweets >= mostLeft.retweets) elem else mostLeft
    }
    else elem
  }
...
trait TweetList {
  ...
  def add(elem: Tweet): TweetList

  def append(list: TweetList): TweetList
...
class Cons(val head: Tweet, val tail: TweetList) extends TweetList {
  ...
  def singleton(elem: Tweet) = new Cons(elem, Nil)
  ...
  def append(list: TweetList): TweetList = {
    new Cons(head, tail append list)
  }
